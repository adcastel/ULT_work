...
do
	...
	cat glt_a_$i.dat | awk '{ sum += $1; count++} END { print sum/count}' >> glt_a_total.dat
	cat glt_m_$i.dat | awk '{ sum += $1; count++} END { print sum/count}' >> glt_m_total.dat
	cat glt_q_$i.dat | awk '{ sum += $1; count++} END { print sum/count}' >> glt_q_total.dat
